...
def create_main_node(panel, coordinate, state, heigth,changing_state = ""):
    middle_heigth = (heigth/2)
    r = create_double_node(panel, Coordinate(x = coordinate.x + 100, y = middle_heigth), state)
    panel.add_line(coordinate.x,  middle_heigth, coordinate.x + 50,  middle_heigth, end_arrow=1, text= changing_state)
    return r
...
def create_node(panel, coordinate, state, changing_state = ""):
    ...
    panel.add_line(coordinate.x, res.y, res.x - width, res.y, end_arrow=1, text = changing_state)
